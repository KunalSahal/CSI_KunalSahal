...
class LoanApprovalPredictor:
    ...
    def preprocess_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['Income_per_Dependent'] = df['Total_Income'] / (df['Dependents'].str.replace('3+', '3').astype(int) + 1)
    ...
    def train_models(self, df: pd.DataFrame, target_column: str = 'Loan_Status') -> Dict[str, Any]:
        """
        ...
        if target_column not in self.encoders:
            ...
            y = self.encoders[target_column].fit_transform(df_processed[target_column])
        else:
            y = self.encoders[target_column].transform(df_processed[target_column])
        
        # Feature selection
        feature_columns = df_processed.columns.drop([target_column, 'Loan_ID'] if 'Loan_ID' in df_processed.columns else [target_column])
        ...
        for name, config in model_configs.items():
            ...
            cv_scores = cross_val_score(self.models[name], X_train_scaled, y_train, cv=5)
            ...
            print(f"{name} - ROC AUC: {roc_auc:.4f}, CV Score: {cv_scores.mean():.4f} (±{cv_scores.std():.4f})")
    ...
    def predict(self, X, model_name='ensemble'):
        ...
        feature_columns = [col for col in X_processed.columns if col not in ['Loan_Status', 'Loan_ID']]
    ...
    def explain_prediction(self, X, instance_idx=0):
        ...
        shap_values = self.shap_explainer.shap_values(X_features.iloc[instance_idx:instance_idx+1])
...
class ModelValidator:
    ...
    def perform_stress_test(model, X_test, perturbation_factor=0.1):
        ...
        for col in numerical_cols:
            noise = np.random.normal(0, perturbation_factor * X_test[col].std(), size=len(X_test))
